...
int game_max_depth = 0;
int game_gen_nodes = 0;
int game_exp_nodes = 0;
...
void DrawWindow() {
    ...
    int attr=0;                       //Variable that displays normal/bold character
...
void DrawWindowState(state_t state) {
    ...
    //Display Pacman
    wattron(win, COLOR_PAIR(Pacman)); mvwaddch(win, state.Loc[4][0], state.Loc[4][1], 'C');
...
void ExitProgram(const char *message) {
    ...
    // printing game stats to output.txt
    exp_nodes_sec = game_exp_nodes/total_search_time;
    char* propagation_type;
    if (propagation == 0) {
        propagation_type = "max";
    }
    else {
        propagation_type = "avg";
    }
    FILE* fp = fopen("output.txt", "w+");
    fprintf(fp, "Propagation = %s\nBudget = %d\nMaxDepth = %d\nTotalGenerated = %d\nTotalExpanded = %d\nTime = %f\nExpanded/Second = %f\nScore = %d", propagation_type, budget, game_max_depth, game_gen_nodes, game_exp_nodes, total_search_time, exp_nodes_sec, Points);
    fclose(fp);
